...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    h1{margin:0 0 8px}
    p{margin:0 0 16px;color:#5d667b}
    ...
    .error{color:#b00020;margin-top:10px;min-height:1.2em}
  </style>
</head>
<body>
  <div class="wrap">
  ...
  <script type="module">
    ...
    // Construye URL absoluta a liga.html (soporta subcarpetas/GitHub Pages)
    const targetUrl = new URL('liga.html', location.href).toString();
      ...
      try{
        ...
        const { error } = await sb.auth.signInWithOAuth({
          ...
          options:{
            ...
            queryParams: { prompt: 'select_account' } // permite elegir cuenta
          }
        });
        if(error) throw error;
      }catch(e){
        setBusy(false);
        err.textContent = 'No se pudo iniciar sesión con Google.';
        console.error(e);
      }
    };
